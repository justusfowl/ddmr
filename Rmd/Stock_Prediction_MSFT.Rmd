...
BA <- getSymbols("BA",from="1990-01-01",auto.assign=FALSE)
stock_data<- BA 
stock_name<- "BA" 
...
save(SVM_model_predict, file = "Microsoft_SVM.rda")
```
We select the parameters from the best workflow with the lowest mse, mae, rmse values and use that variant to train.
...
save(RF_model_predict, file = "Microsoft_RF.rda")
```
...
save(GBM_model_predict, file = "Microsoft_GBM.rda")
```
