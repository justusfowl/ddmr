...
AAPL <- getSymbols("AAPL",from="1990-01-01",auto.assign=FALSE)
stock_data<- AAPL 
stock_name<- "AAPL" 
...
save(SVM_model_predict, file = "Apple_SVM.rda")
```
We select the parameters from the best workflow with the lowest mse, mae, rmse values and use that variant to train.
...
save(RF_model_predict, file = "Apple_RF.rda")
```
...
save(GBM_model_predict, file = "Apple_GBM.rda")
```

```{r}
validation_data1<-validation_data
```
AAPLy the trained model on the validation data 
```{r}
# SVM
